...
public class MinRectangle {
	...
	public static void main(String[] args) {
		...
		System.out.println(sl.solution(arr3));
	}
}
...
class Solution7 {
    public int solution(int[][] sizes) {
    	...
    	for( int i = 0; i < sizes.length; i++) {
    		if( i == 0 ) {
    			top = Math.max(sizes[i][0], sizes[i][1]);
    			bot = Math.min(sizes[i][0], sizes[i][1]);
    		}
    		else {
        		if(top < Math.max(sizes[i][0], sizes[i][1])) {
        			top = Math.max(sizes[i][0], sizes[i][1]);
        		}
        		if(bot < Math.min(sizes[i][0], sizes[i][1])) {
        			bot = Math.min(sizes[i][0], sizes[i][1]);
        		}
    		}
    	}
